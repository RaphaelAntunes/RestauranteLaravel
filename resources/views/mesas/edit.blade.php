...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Editar Mesa {{ $mesa->numero }}</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Atualize as informações da mesa</p>
        </div>
        ...
        <form action="{{ route('mesas.update', $mesa) }}" method="POST">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="number" name="numero" id="numero" value="{{ old('numero', $mesa->numero) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="capacidade" id="capacidade" value="{{ old('capacidade', $mesa->capacidade) }}" required min="1"
                        class="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 dark:focus:ring-red-400 focus:border-transparent transition-colors">
                </div>

                <div>
                    <label for="localizacao" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Localização</label>
                    <input type="text" name="localizacao" id="localizacao" value="{{ old('localizacao', $mesa->localizacao) }}"
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="manutencao" {{ old('status', $mesa->status) == 'manutencao' ? 'selected' : '' }}>Manutenção</option>
                    </select>
                </div>

                <div class="flex items-center pt-2">
                    <input type="checkbox" name="ativo" id="ativo" value="1" {{ old('ativo', $mesa->ativo) ? 'checked' : '' }}
